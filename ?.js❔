...
sock.ev.on('call', async (calls) => {
    ...
    sock.ev.on('messages.upsert', async (chatUpdate) => {
        try {
            ...
            // ─────────────────────────────────────────────────────────────
            // Replies to every message when enabled, respects marker law.
            // The marker check is the first line inside handleIncomingMessage.
         //   const { handleIncomingMessage } = require('./src/Commands/AI/chatbot.js');
