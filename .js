...
sock.sendMessage = async (jid, content, options = {}) => {
    try {
        ...
        // AI Badge - ONLY in private chats (DMs) - HANDLES BOTH @lid and @s.whatsapp.net
        const aiEnabled = getVar('AI_BADGE', true); // Default ON
        
        // Check if this is a private chat (DM) - supports both LID and standard JID
        ...
        //  DEBUG LOG
     //   console.log('[AI BADGE DEBUG]', {
         //   jid: jidStr,
      //      aiEnabled,
      //      isPrivateChat,
       //     willApply: aiEnabled && isPrivateChat
     //   });
        
        //  Apply AI badge if conditions met
        ...
        // Skip if explicitly disabled or if it's a plain text message
    ...
    // ── GROUP EVENTS PATCH (independent welcome/goodbye toggles) ──
...
try {
    ...
    // Check extended text (quoted replies, mentions, etc.)
    else if (mek.message?.extendedTextMessage?.text) {
        msgText = mek.message.extendedTextMessage.text.trim();
    }
    // Fallback to m.body or m.text
    else {
        msgText = (m.body || m.text || '').trim();
    }
    
    // Also check if it's just the emoji (strip any hidden chars)
    const cleanText = msgText.replace(/[\u200B-\u200D\uFEFF]/g, '').trim();
    
    // Try matching with both raw and cleaned text
