...
module.exports = {
    name: "bury",
    category: "media",
    desc: "Apply burial effect to an image or a user's profile picture",
    usage: ".burial [@user] or reply to an image/user",
    ...
    execute: async (sock, m, { args, reply }) => {
        ...
        // 2) Check if a user is mentioned or quoted contact
            ...
            // Try to parse a number from args (e.g., .burial 234XXXXXXXXXX)
            else if (args[0]) {
        ...
        try {
            const result = await applyEffect(targetBuffer, "burial");
            ...
            console.error("[BURIAL ERROR]", err);
            reply("_*ಠ_ಠ Failed to apply burial effect.*_");
        }
    }
};