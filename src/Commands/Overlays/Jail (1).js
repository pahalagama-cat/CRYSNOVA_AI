...
module.exports = {
    name: "jail",
    category: "media",
    desc: "Apply jail effect to an image or a user's profile picture",
    usage: ".jail [@user] or reply to an image/user",
    ...
    execute: async (sock, m, { args, reply }) => {
        ...
        // 2) Check if a user is mentioned or quoted contact
            ...
            // Try to parse a number from args (e.g., .jail 234XXXXXXXXXX)
            else if (args[0]) {
        ...
        try {
            const result = await applyEffect(targetBuffer, "jail");
            ...
            reply("_*ಠ_ಠ Failed to apply jail effect.*_");
        }
    }
};