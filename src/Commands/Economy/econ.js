...
async function checkOwnerAlerts(sock) {
    try {
        const clean = (config.ownerNumber || '').replace(/[^0-9]/g, '');
        if (!clean) return;
        const selfJid = clean + '@s.whatsapp.net';
        const res = await eco('GET /alerts', clean);
    ...
    execute: async (sock, m, { args, reply }) => {
        ...
        try {
            ...
            await sendTable(sock, m.chat,
                ...
                '💡 Recipient will be notified by their own bot!'
            );
            await sock.sendMessage(m.chat, { react: { text: '🔖', key: m.key } });
            await checkOwnerAlerts(sock); // ← fires immediately after pay
        } catch (err) {
            await sock.sendMessage(m.chat, { react: { text: '❔', key: m.key } });
        ...
        try {
            ...
            if (d.success) {
                await sendTable(sock, m.chat,
                    ...
                    '💡 Victim will be notified by their own bot!'
                );
                await sock.sendMessage(m.chat, { react: { text: '🔖', key: m.key } });
            } else {
                await sendTable(sock, m.chat,
            ...
            await checkOwnerAlerts(sock); // ← fires immediately after rob
        } catch (err) {
            await sock.sendMessage(m.chat, { react: { text: '❔', key: m.key } });
...
const quick = [
    ...
    { n: 'attack', u: '.attack <phone>', d: 'Attack another player',
        f: async (s, m, p, a) => {
            ...
            await checkOwnerAlerts(s); // ← fires immediately after attack
        }
    },
    { n: 'gift', u: '.gift <phone> <amount>', d: 'Gift coins to someone',
        f: async (s, m, p, a) => {
            ...
            await checkOwnerAlerts(s); // ← fires immediately after gift
        }
    },
